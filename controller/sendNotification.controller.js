			...
			if (err) {
				...
				.exec((err,project)=>{					
					...
					notification.save(function(err,SavedUser){
						...
						var output = `<!doctype html><html><head><title> title111</title></head><body><div style="width:100%;margin:0 auto;border-radius: 2px;box-shadow: 0 1px 3px 0 rgba(0,0,0,.5);border: 1px solid #d3d3d3;background:#e7eaf0;"><div style="border:10px solid #3998c5;background:#fff;margin:25px;"><center><span style="font-size:30px;color:#181123;"><b>Rao Infotech</b></span></center>	<background img><div style="width:85%;margin:0 auto;border-radius:4px;border:1px solid white;background-color:white;box-sizing: border-box; "><div style="margin-left:30px;padding:0;"><p style="color:black;font-size:15px;"> `+req.body.pmanagerName+` project manager of ` +project[0].title+` notified to all team member that `+req.body.content+`.</p></div></div></div></body></html>
						`;
						var mailOptions = {
							from: 'tnrtesting2394@gmail.com',
							to: maillist,
